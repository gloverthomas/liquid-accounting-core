...
test.describe("LIQ cross-repo parity seams", () => {
  ...
  test("Core deep-links to #revenue-summary; Reporting opens Revenue summary (LIQ-9)", async ({ page }) => {
    await page.goto("/");
    ...
    const dashboardTask = page.getByRole("link", { name: /Review Revenue summary/i });
    await expect(dashboardTask).toHaveAttribute("href", /#revenue-summary$/);
